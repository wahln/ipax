...
| **bound-only** composite least-squares at RT scale (fluence-map shape, `f(Dx)` with box bounds, no constraint rows) making slow per-iteration progress under L-BFGS | `LBFGSOptions(seed_formula="scalar1", memory=20)` (memory up to 50 if RAM allows); keep the default monotone μ; optionally add `backtrack_interpolation=True` | the direct seed is over-stiff on chained least squares (same misalignment mechanism as the KKT-plateau row) and the default window is too short for the ill-conditioned `DᵀD` spectrum — measured recipe and numbers on the [TROTS page](../benchmarks/trots.md#bound-only-fluence-map-runs) |
